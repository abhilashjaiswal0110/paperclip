/**
 * Schema and structure validation tests for the AJT Technologies
 * agentcompanies/v1 company package located at agents/ajt/.
 *
...
const ROOT = path.resolve(__dirname, "../../agents/ajt");
...
describe("COMPANY.md frontmatter", () => {
  ...
  it("declares optional secret ANTHROPIC_API_KEY under requirements.secrets.optional", () => {
...
const TEAM_SLUGS = ["executive", "engineering", "product", "marketing"] as const;
...
// ---------------------------------------------------------------------------
// 6. Product MVP project + seed tasks
// ---------------------------------------------------------------------------
...
describe("Product MVP project", () => {
  it("PROJECT.md exists", () => {
    expect(fileExists("projects/product-mvp/PROJECT.md")).toBe(true);
  });

  for (const task of SEED_TASKS) {
    const relPath = `projects/product-mvp/tasks/${task}/TASK.md`;
    it(`${relPath} exists`, () => {
      expect(fileExists(relPath)).toBe(true);
    });
...
// ---------------------------------------------------------------------------
// 7. Recurring tasks — portable (recurring: true, no schedule block, no startsAt)
// ---------------------------------------------------------------------------
...
describe("Recurring task portability", () => {
  for (const task of RECURRING_TASKS) {
    ...
    it(`${relPath} does NOT contain a startsAt timestamp`, () => {
      const content = readFile(relPath);
...
describe(".paperclip.yaml", () => {
  ...
  it("GH_TOKEN is declared as required for CEO", () => {
    expect(content).toMatch(/GH_TOKEN:/);
    expect(content).toMatch(/requirement:\s*required/);
  });

  it("FIGMA_API_KEY is declared as optional for ux-designer", () => {
    expect(content).toMatch(/FIGMA_API_KEY:/);
    expect(content).toMatch(/requirement:\s*optional/);
  });

  it("declares all 3 routines", () => {
    for (const task of RECURRING_TASKS) {
      expect(content).toMatch(new RegExp(`${task}:`));
    }
  });
});

// ---------------------------------------------------------------------------
// 9. README.md — structure checks
// ---------------------------------------------------------------------------
...
describe("README.md", () => {
  ...
  it("has Current Project section", () => {
    expect(content).toMatch(/## Current Project/);
  });

  it("has Directory Structure section", () => {
    expect(content).toMatch(/## Directory Structure/);
  });

  it("has Routines table", () => {
    expect(content).toMatch(/## Routines/);
  });
});
